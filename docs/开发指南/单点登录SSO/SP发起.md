...
#### 1、访问应用页面
用户到 SP 应用页面，会在 SP 通过 Redirect 或 POST 提交一个某种认证协议如 SAML / CAS 的挑战请求， 在 Portal 登录后， 再跳转回 SP 作为应答， 实现统一认证，如下图所示：
...
（4）浏览器携带 SAML AuthnRequest 请求，访问定义好的 Portal 中 SP 发起的 SSO URL

（5）Portal 验证 SAML AuthnRequest，若该用户已登录直接跳至步骤（9），否则继续步骤（6）

（6）重定向到 Portal 登录页面

（7）用户输入 Portal 的账户和密码

（8）浏览器携带账号密码请求登录 Portal

（9）登录 Portal 后， Portal 分析 SAML AuthnRequest 中的 SP 应用信息，并获取更多用户信息，然后组合生成响应包含 RelayState 的 SAML Response Token。
...
#### 2、访问 SP 资源
用户访问 SP 资源，SP 会重定向到 Portal 的 SSO 地址，在 Portal 认证通过后，会向 SP 返回 SP 发起登录的页面并携带请求事件redirect_url参数，用户登录后 SP会向浏览器返回并显示用户访问的资源页面，如下图所示：
...
（6）用户输入 Portal 的账户和密码进行登录

（7）浏览器携带账号密码请求登录 Portal
...
### 接口后置
SP 登录时使用 Portal 进行接口后置认证登录场景，如下图所示：
...
（7）SP 1 使用 Portal 接口进行认证，将用户名、密码传递给 Portal 进行认证登录

（8）Portal 进行认证后，生成主 token ，并返回主 token 和应用列表以及用户信息给 SP1
...
（12）浏览器携带主 token 和应用id，向 Portal 请求生成子 token

（13）Portal 返回 SP2 的子 token 以及 SP2 重定向地址
...
### 登录跳转
用户到 SP 应用页面，会跳转到 Portal 登录页面进行统一认证，登录后，返回一个包含更多用户信息 JWT token 到 SP 页面，包括应用列表和访问一个 SP 应用的子token，从而实现单点登录。如下图所示：
...
（6）浏览器携带用户提交的用户名密码以及应用id，向 Portal 请求登录

（7）Portal 本地认证成功后，创建主 session ，并返回应用列表的 JWT token 内含了子 token
...
（15）Portal 由于主 session 已创建，可直接根据应用id生成 SP2 系统的子 token ，并返回子 token
...
而在 Portal 中， 一个 CAS （标准）应用实现了标准的 CAS 流程。它充当一个 CAS Server的角色。当 CAS client 决定使用 CAS （标准）应用作为 CAS Server 时。在登录认证时需要使用 Portal 系统中公司的主账号，密码进行认证。

### Portal 中添加 CAS （标准）应用
`CAS 标准应用目前只能由 IT 管理员 在应用添加菜单中添加。下面是 IT 管理员的应用添加流程。如果希望使用 CAS（标准）单点登录，可以请管理员进行协助添加和配置。`<br>
1、以IT管理员身份登录 Portal ，点击添加应用,找到 CAS （标准），点击 添加应用
